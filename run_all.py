...
-----
  ...
  13 RHOA-3 direct          — Phase 4A × 3 global seeds [~44h/seed, skip with --skip-heavy]
  14 RHOA-2 isolation       — Run C: RHOA-1✓ RHOA-2✗ [~44h, skip with --skip-heavy]
  15 recovery experiment    — Formal LURE recovery framing (analytical, fast)
  16 RHOA report            — LaTeX tables, claims, checklist (runs LAST: collects all TABLE_*.tex)

Usage
-----
  python run_all.py                  # run everything (scripts 1–16)
...
---------------------
  ...
  Then run scripts 13 and 14 separately when compute time is available:
  python run_all.py --only 13
  python run_all.py --only 14
  python run_all.py --only 16        # regenerate report with new tables
"""
...
SCRIPTS = [
    ...
    (13, "rhoa3_direct",    "13_rhoa3_direct_validation.py"),   # compute-heavy (~132h total)
    (14, "rhoa2_isolation", "14_rhoa2_isolation.py"),           # compute-heavy (~44h)
    (15, "recovery",        "15_recovery_experiment.py"),       # analytical (fast)
    (16, "rhoa_report",     "09_rhoa_report.py"),               # LAST: collects all TABLE_*.tex
]

# Scripts that involve full Phase 4A re-runs (~12h each)
HEAVY_SCRIPTS = {13, 14}
N5_SCRIPT     = 10
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "RHOA-3 direct (~36h) and RHOA-2 isolation (~12h)"
        ),
    )
    parser.add_argument(
        "--only", nargs="+", type=int,
    ...
    if skipped:
        print(f"\n  Skipped (run separately): {', '.join(skipped)}")
        print("    python run_all.py --only 13   # RHOA-3 direct (~36h)")
        print("    python run_all.py --only 14   # RHOA-2 isolation (~12h)")
    ...
    if n_fail == 0:
        ...
        if skipped:
            print("    4. Run scripts 13+14 when compute is available, then re-run 16")
    else:
